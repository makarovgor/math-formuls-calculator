package resultWork.parser

object parser_failed {

  //https://stackoverflow.com/questions/5805496/arithmetic-expression-grammar-and-parser
  ...
  val formulaParser = new FormulaParser(constants = Map("x" -> x))

  println(formulaParser.evaluate("pow(3, 5)")) // 17.0
//  println(formulaParser.evaluate("height*perimeter(radius)")) // 502.6548245743669
//  println(formulaParser.evaluate("m/sqrt(1-v^2/c^2)")) // 80.00000000003

//  val formulaParser = new FormulaParser(Set("x"), unary = Map(
//    "sin" -> (math.sin(_)), "cos" -> (math.cos(_)), "tan" -> (math.tan(_))
//  ))
//  val formula = formulaParser.evaluate("( x + 2 ) / 2")
